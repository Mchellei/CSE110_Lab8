describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const sR = await page.evaluateHandle((element) => element.shadowRoot, prod); 
      const button1 = await sR.$('button'); 
      await button1.click(); 
    const productItem = await page.$$('product-item'); 
    for (const product of productItem) {
      const shadowRoot = await page.evaluateHandle((element) => element.shadowRoot, product); 
      const button = await shadowRoot.$('button'); 
      await button.click(); 
    }
